...
const checkoutPath = resolve('src/checkout.js')

const fixed = `export class CheckoutValidationError extends Error {
...
const regression = `export function quoteCart(cart) {
...
if (state === 'fixed') {
  await writeFile(checkoutPath, fixed)
  console.log('checkout.js switched to fixed validation state')
} else if (state === 'regression') {
  await writeFile(checkoutPath, regression)
  console.log('checkout.js switched to regression state')
} else {
  console.error('Usage: node scripts/write-checkout-state.mjs <fixed|regression>')
  process.exit(1)
}
